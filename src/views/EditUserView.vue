...
export default {
  ...
  mounted() {
    ...
    axios
      .get(
        "https://backend-control-inv.herokuapp.com/api/users/id/" + this.id,
        {
          headers: {
  ...
  methods: {
    ...
    addPro() {
      axios
        .put(
          "https://backend-control-inv.herokuapp.com/api/users/" + this.id,
          this.form,
          {
            headers: {
              "user-token": localStorage.token,
        ...
        .then((data) => {
          if (data.data.errors) {
            ...
            const user = datos.find(element => element.id == this.id);
